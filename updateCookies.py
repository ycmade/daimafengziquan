...
def login(username, password):
    url = "http://192.168.0.2:5700/api/login?t=%s" % gettimestamp()
...
def getitem(key):
    url = "http://192.168.0.2:5700/api/envs?searchValue=%s&t=%s" % (key, gettimestamp())
    r = s.get(url)
...
def getckitem(key):
    url = "http://192.168.0.2:5700/api/envs?searchValue=JD_COOKIE&t=%s" % gettimestamp()
    r = s.get(url)
...
def wstopt(cookies):
    headers = {
        'user-agent': 'JD4iPhone/10.1.2 CFNetwork/1240.0.4 Darwin/20.6.0;',
    ...
    url = 'https://api.m.jd.com/client.action?functionId=genToken&clientVersion=10.1.2&build=167802&client=apple' \
          ...
          '=zh_CN&uuid=5981174f3a0a886d442df9d4398c846458b8a1fe&aid=a27b83d3d1dba1cc&area=19_1601_36953_50397&networkType=wifi&wifiBssid=&uts' \
...
def update(text, qlid):
    url = "http://192.168.0.2:5700/api/envs?t=%s" % gettimestamp()
